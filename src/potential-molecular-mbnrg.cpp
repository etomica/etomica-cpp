...
double PotentialMolecularMBnrg::u(Box& box, int iMolecule, int jMolecule)
{
    ...
    if (energy2b < 100000)
    {
        ...
        if (std::isnan(total_energy) || total_energy < -5000 || energy2b < 0)
        {
            ...
            for (int j = 0; j <= 2; j++)
            {
                ...
                for (int i = 3; i <= 5; i++)
                {
                    double* q = box.getAtomPosition(i);
                    double dr[3];
                    for (int k=0; k<3; k++) dr[k] = p[k]-q[k];
                    double r2 = Vector::squared(dr);
                    minDist = std::min(minDist, r2);
