...
PotentialMolecularAtomicC::PotentialMolecularAtomicC(SpeciesList& sl, Potential& p) : potential(p), speciesList(sl)
{
}

double PotentialMolecularAtomicC::u(Box& box, int iMolecule, int jMolecule) {
  ...
  double* com = box.getAtomPosition(iFirstAtom);
  ...
  com = box.getAtomPosition(jFirstAtom);
